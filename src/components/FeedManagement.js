...
const FeedManagement = ({ farmId }) => {
  ...
  const categoryFeedPlans = animals.reduce((acc, animal) => {
    ...
    if (!acc[cat]) {
      acc[cat] = {
        count: 0,
        feedPlan: animal.feed_plan || { feed_type: "Not set", daily_amount_kg: 0, frequency: "N/A", notes: "No plan defined" },
      };
    }
    acc[cat].count += 1;
    return acc;
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell><strong>Daily Amount (kg)</strong></TableCell>
            <TableCell><strong>Frequency</strong></TableCell>
            <TableCell><strong>Notes</strong></TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {Object.entries(categoryFeedPlans).map(([category, { count, feedPlan }]) => (
